...
```bash
pnpm install
pnpm --filter app-designer-vscode run compile   # build once
pnpm --filter app-designer-vscode run watch     # rebuild on changes
pnpm --filter app-designer-vscode run package   # produce a .vsix
```
...
- Initial schema validation and sample command.
